...
<template>
  <button
    ...
    class="group link inline-block p-1 text-gray-600 hover:text-gray-700 dark:hover:text-gray-300 sm:p-2"
    @click="toggleDark()"
  >
      ...
      <ClientOnly>
        <template v-if="color.value === 'dark'">
          <Icon
            ...
            class="theme-icon icon--on dark:text-light-100"
          />
          <Icon
            name="line-md:moon"
            class="theme-icon icon--off dark:text-light-100"
          />
        </template>
        <template v-else>
